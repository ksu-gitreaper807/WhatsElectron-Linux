...
To wipe that login (and everything else the app stored locally): `npm run reset`.
...
- **TLS-intercepting proxies.** Packaging downloads the Electron runtime and helper tools
  from GitHub releases. Behind a proxy that re-signs TLS (corporate MITM, some CI images)
  Node rejects the certificate and the build dies with `unable to verify the first
certificate`. Point Node at your root CA: `NODE_EXTRA_CA_CERTS=/path/to/ca.pem npm run dist:linux`.
...
| Remove every trace of a login                   | `npm run reset` deletes the whole local profile (`~/.config/whatsapp-desktop`): session, settings, window state, logs and caches. `npm run reset -- --session-only` logs out but keeps settings. The repository itself never stores session data, so there is nothing to clean in a clone. |
